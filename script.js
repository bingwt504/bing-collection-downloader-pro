...
function attachCollectionBtn() {
  ...
  //v2.0 changed the background-color
  style.innerHTML = `
    .${hoverClass} {
      cursor: pointer;
...
//v2.0 +
  const htmlContentHeader = `<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <table bgcolor="black" width="1200">
        <tr bgcolor="grey">
            <th width="500">Prompt</th>
            <th width="550">Image</th>
        </tr>
`;

  var singleHtmlContent = "";
  var totalHtmlContent = [];
  const totalHtmlFileName = "prompts_file.html";
  const arrayColumn = (arr, n) => arr.map(x => x[n]);

  // For the Copy button
  const copyFunctionHtmlString = `
    <script>
    function myCopyText(elementId) {
  ...
  const htmlContentFooter = `    </table>
  ${copyFunctionHtmlString}
</body>
</html>`;
//v2.0 -
...
//v2.0 start edit
        ...
        const comment = imageFileName;

        singleHtmlContent = `    <tr bgcolor="lightgrey" align="center">
    <td>${num}.
    <br><br><span id="myPromptContent${num}">${promptContent}</span>
    <br><br><button onclick="myCopyText('myPromptContent${num}')">Copy text</button>
    </td>
    <td>
...
//v2.0 start edit
        ...
        totalHtmlContent.push([num, `${singleHtmlContent}`]);
        totalHtmlContent.sort(function(a,b) {
                                return a[0]-b[0]
                             });;
        zip.file(totalHtmlFileName, htmlContentHeader + arrayColumn(totalHtmlContent, 1).join('') + htmlContentFooter);
                ...
                console.warn(`Failed to download image: ${promptContent}`);
                thumbnails.forEach(
                async ({thumbnailUrl}, i) => {
                    ...
                    if (response.ok) {
                        ...
                        // Image file name
                        const imageFileName = `${num}-${i} ${shortFileName}.jpg`;
                        ...
                        singleHtmlContent = `    <tr bgcolor="lightgrey" align="center">
    <td>${num}-${i}.
    <br><br><span id="myPromptContent${num}-${i}">${promptContent}</span>
    <br><br><button onclick="myCopyText('myPromptContent${num}-${i}')">Copy text</button>
    </td>
    <td>
      <img src="${imageFileName}"
           width = "215"
...
`;
                        ...
                        htmlFileName = `${num}-${i}_image.html`;
                        htmlContent = `${htmlContentHeader}${singleHtmlContent}${htmlContentFooter}`;
                        zip.file(htmlFileName, htmlContent);
            ...
            console.warn(`Failed to download thumbnail: ${promptContent}`);
